...
type TerritoryId = 'crockett1986'|'wwf1985_90'|'combined';
type MatchType = 'singles'|'tag'|'sixManTag'|'eightManTag'|'battleRoyal'|'royalRumble'|'survivorSeries'|'survivorSeries4'|'survivorSeries5'|'tagBattleRoyal'|'bunkhouseBattleRoyal'|'warGames';
type MatchRule = 'Standard'|'No DQ'|'Steel Cage'|'Two out of Three Falls'|'Survivor Series Elimination'|'WarGames'|'Bunkhouse Rules';
type SimMode = 'full'|'quick';
type MoveLetter = 'A'|'B'|'C'|'D'|'E';
type PinMode = 'standard'|'optionB';
type Move = { from:number; to:number; name:string; bonus:number; letter?:MoveLetter; pin?:'setup'|'auto' };
type Wrestler = { id:string; name:string; pp:number; rank:number; territory:TerritoryId|'both'; qpr:string; style:string; brAdj:number; moves:Move[] };
type Log = { cls?:string; html:string };
type NotableEvent = { category:string; text:string };
type SideState = { wrestler:Wrestler; currentPP:number; startPP:number; startLevel:number; pending?:Move|null; finishers:number; nearfalls:number; blood:boolean; controlPenalty:number };
type TeamState = { name:string; official:boolean; finisher:string; members:[SideState,SideState]; active:0|1; breakups:number; hotTagUsed:boolean; miscueRisk:number };
type Match = { a:SideState; b:SideState; timeLimit:number; moveLimit:number; movesUsed:number; result:string|null; winner?:string; logs:Log[]; controlSwings:number; lastControl?:string; rating?:number; drama:number; goat:boolean; pinMode:PinMode; territory:TerritoryId; officialMatch:boolean; pointsApplied?:boolean; kind?:'singles'|'tag'; rule?:MatchRule; simMode?:SimMode; tag?:{ teamA:TeamState; teamB:TeamState }; notables:NotableEvent[]; lastFinishMove?:string };;
type CardMatch = { id:number; type:MatchType; label:string; a?:string; b?:string; teamA?:string[]; teamB?:string[]; teamAPreset?:string; teamBPreset?:string; entrants:string[]; entrantMode?:'random'|'selected'; entrantCount?:number; mode?:SimMode; rule?:MatchRule; result?:string; rating?:number; winnerId?:string; loserId?:string };
type EventType = 'House Show'|'TV Show'|'Premium TV'|'Small PPV'|'Major PPV'|'Biggest PPV';
type EventCard = { name:string; date:string; venue:string; eventType:EventType; territory:TerritoryId; matches:CardMatch[]; history:string[] };
type RosterRelations = Record<string,{allies:string; enemies:string}>;
type Feud = { id:string; a:string; b:string; heat:number; note:string; active:boolean };
type MatchHistoryEntry = { id:string; date:string; territory:TerritoryId; eventName:string; eventType?:EventType; venue?:string; matchType:string; result:string; participants:string[]; winnerId?:string; rating?:number; notables?:string[]; eliminations?:Record<string,string[]>; entryOrder?:string[] };
...
function matchupKey(a:string,b:string){ return [a,b].sort().join(' vs '); }
...
function isHouseOrRegularTv(){ return eventCard.eventType==='House Show' || eventCard.eventType==='TV Show'; }
function isMajorCard(){ return ['Premium TV','Small PPV','Major PPV','Biggest PPV'].includes(eventCard.eventType); }
function isMainEventer(w:Wrestler){ return wrestlerRank(w,currentTerritory)>0 && (wrestlerRank(w,currentTerritory)<=12 || qprValue(w.qpr)>=6); }
function isPrelim(w:Wrestler){ return qprValue(w.qpr)<=2 || w.rank>=50 || w.pp<=180; }
...
const stars = (n:number) => '★'.repeat(Math.floor(n)) + (n%1>=.5?'½':'');
const moveLimitFromMinutes = (min:number) => min < 0 ? 9999 : min*3;
const fmtTime = (moves:number) => `${String(Math.floor(moves/3)).padStart(2,'0')}:${String((moves%3)*20).padStart(2,'0')}`;


const venueCatalog:Record<TerritoryId,{name:string; city:string; note?:string}[]> = {
 crockett1986: [
...
function venueOptions(){
 const venues=venueCatalog[currentTerritory] || venueCatalog.combined;
 const current=eventCard.venue;
 const hasCurrent=venues.some(v=>`${v.name} — ${v.city}`===current || v.name===current);
 const rows=[...venues];
 const customCurrent=current && !hasCurrent ? `<option value="${current}" selected>${current}</option>` : '';
 return customCurrent + rows.map(v=>{
  const value=`${v.name} — ${v.city}`;
  const selected=current===value || current===v.name ? 'selected' : '';
...
const pinCharts:Record<PinMode, Record<MoveLetter, Record<string,[number,number,number]>>> = {
 standard:{
...
const genericMoves = (name:string, finisher:string):Move[] => [
...
const crockett:Wrestler[] = crockettSeed.map(([rank,name,pp,qpr,fin,brAdj])=>({id:'c-'+name.toLowerCase().replace(/[^a-z0-9]+/g,'-'),name,pp,rank,territory:'crockett1986' as TerritoryId,qpr,style:qpr,brAdj,moves:genericMoves(name,fin)}));
...
const wwf = [...wwfSeed, ...wwfExtraSeed].map(([rank,name,pp,qpr,fin,brAdj])=>({id:'w-'+name.toLowerCase().replace(/[^a-z0-9]+/g,'-'),name,pp,rank,territory:'wwf1985_90' as TerritoryId,qpr,style:qpr,brAdj,moves:genericMoves(name,fin)}));
const allWrestlers = [...crockett,...wwf];

const STORAGE_KEY = 'squaredCircleTerritoryPointsV8';
type StoredPoints = { wrestlers:Record<string,number>; teams:Record<string,number> };
...
type TagTeamTemplate = { id:string; name:string; members:[string,string]; territory:TerritoryId|'both'; finisher:string; rating?:'A'|'B'|'C'|'D'; rankingPoints?:number; note?:string };
type Manager = { id:string; name:string; territory:TerritoryId|'both'; stable:string[]; note?:string };
const officialTagTeams:TagTeamTemplate[] = [
...
let eventCard:EventCard = { name:'Saturday Night Card', date:'1987-01-19', venue:'Madison Square Garden', eventType:'TV Show', territory:'wwf1985_90', matches:[], history:[] };
...
function qprValue(qpr:string){ const m=qpr.match(/[A-G]/); return m ? (8-(m[0].charCodeAt(0)-64)) : 1; }
function alignment(qpr:string){ if(qpr.includes('Heel')) return 'Heel'; if(qpr.includes('Face')) return 'Face'; return 'F/H'; }
...
function cardTeamSize(t:MatchType){
 if(t==='tag') return 2;
...
function isCardTeamType(t:MatchType){ return ['tag','sixManTag','eightManTag','survivorSeries','survivorSeries4','survivorSeries5','warGames'].includes(t); }
...
function validateUniqueCardParticipants(ids:string[], needed:number){
 if(ids.length<needed) return false;
 return new Set(ids).size===ids.length;
}
function cardTypeName(t:MatchType){
 const names:Record<MatchType,string>={singles:'Singles',tag:'Tag Team',sixManTag:'6-Man Tag',eightManTag:'8-Man Tag',battleRoyal:'Battle Royal',royalRumble:'Royal Rumble',survivorSeries:'Survivor Series 4 vs 4',survivorSeries4:'Survivor Series 4 vs 4',survivorSeries5:'Survivor Series 5 vs 5',tagBattleRoyal:'Tag Team Battle Royal',bunkhouseBattleRoyal:'Bunkhouse Battle Royal',warGames:'WarGames'};
 return names[t] || t;
}
...
function isCardBattleType(t:MatchType){ return t==='battleRoyal' || t==='royalRumble' || t==='bunkhouseBattleRoyal' || t==='tagBattleRoyal'; }
