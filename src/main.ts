...
function makeSuggestedMatches(){
 ...
 const activeRosterIds=new Set(r.map(w=>w.id));
 const suggestions:{title:string; reason:string; a:string; b:string; kind:'feud'|'ranking'|'relation'|'enhancement'|'warning'}[]=[];
 activeFeuds()
  .filter(f=>!!f.a && !!f.b && activeRosterIds.has(f.a) && activeRosterIds.has(f.b))
  .sort((a,b)=>b.heat-a.heat)
  .slice(0,4)
  .forEach(f=>{
   suggestions.push({title:`${wrestlerName(f.a)} vs ${wrestlerName(f.b)}`,reason:`Active feud · heat ${f.heat}${f.note?` · ${f.note}`:''}`,a:f.a,b:f.b,kind:'feud'});
  });
...
function normalizeWrestlerId(value?:string){
 if(!value) return value;
 if(isConditionalRef(value)) return value;
 if(byId(value)) return value;
 const match=roster().find(w=>w.name.toLowerCase()===value.toLowerCase());
 return match?.id || value;
}

function syncCardMatchParticipants(m:CardMatch){
 if(m.type==='singles'){
  m.a=normalizeWrestlerId(m.a);
  m.b=normalizeWrestlerId(m.b);
  if(m.teamA?.[0]) m.teamA[0]=normalizeWrestlerId(m.teamA[0]) || m.teamA[0];
  if(m.teamB?.[0]) m.teamB[0]=normalizeWrestlerId(m.teamB[0]) || m.teamB[0];
  if(m.a) m.teamA=[m.a];
  else if(m.teamA?.[0]) m.a=m.teamA[0];
  if(m.b) m.teamB=[m.b];
  else if(m.teamB?.[0]) m.b=m.teamB[0];
  if(m.teamA) m.teamA=m.teamA.slice(0,1);
  if(m.teamB) m.teamB=m.teamB.slice(0,1);
 }
}

function clearCardMatchResult(m:CardMatch){
 m.result=undefined;
 m.rating=undefined;
...
function slotPositionLabel(index:number){
 if(index===0) return 'Opening Match';
 if(index===eventCard.matches.length-1) return 'Main Event';
 if(index===eventCard.matches.length-2 && eventCard.matches.length>3) return 'Semi-Main Event';
 return `Match ${index+1}`;
}

function generatedMatchLabel(m:CardMatch){
 syncCardMatchParticipants(m);
 if(isCardBattleType(m.type)){
  const count=m.entrantMode==='selected' ? (m.entrants||[]).length : (m.entrantCount||20);
  if(m.type==='royalRumble') return `${count}-Man Royal Rumble`;
  if(m.type==='tagBattleRoyal') return 'Tag Team Battle Royal';
  if(m.type==='bunkhouseBattleRoyal') return 'Bunkhouse Battle Royal';
  return `${count}-Man Battle Royal`;
 }
 const a=plainText(cardSideSummary(m,'A')) || 'TBD';
 const b=plainText(cardSideSummary(m,'B')) || 'TBD';
 const base=`${a} vs. ${b}`;
 if(m.rule && m.rule!=='Standard' && !['Survivor Series Elimination','WarGames'].includes(m.rule)) return `${m.rule}: ${base}`;
 if(m.type==='warGames') return `WarGames: ${base}`;
 if(m.type==='survivorSeries4' || m.type==='survivorSeries' || m.type==='survivorSeries5') return `Survivor Series Elimination: ${base}`;
 return base;
}

function shouldAutoLabel(m:CardMatch){
 if(m.labelMode==='custom') return false;
 if(m.labelMode==='auto') return true;
 return !m.label || /^Match \d+$/i.test(m.label) || /^(Opening Match|Semi-Main Event|Main Event)$/i.test(m.label);
}

function updateAutoLabel(m:CardMatch){
 if(shouldAutoLabel(m)){
  m.labelMode='auto';
  m.label=generatedMatchLabel(m);
 }
}

function displayMatchLabel(m:CardMatch, index:number){
 updateAutoLabel(m);
 return (m.label || '').trim() || slotPositionLabel(index);
}
...
function renderSavedResults(){
 if(!eventCard.history.length) return '<div class="tiny">No results yet.</div>';
 return `<div class="savedResultCards">${eventCard.history.map((h,idx)=>{
  const raw=plainText(String(h || ''));
  const spaced=raw
   .replace(/(.*?)(QUICK TAG RESULT|QUICK MATCH RESULT|FULL MATCH RESULT|RESULT):?\s*/i, (_all,title,mode)=>`${String(title).trim()}|||${String(mode).trim()}|||`)
   .replace(/\s+/g,' ')
   .trim();
  const parts=spaced.split('|||');
  const hasMode=parts.length>=3;
  const title=(hasMode ? parts[0] : `Result ${idx+1}`).replace(/:$/, '').trim() || `Result ${idx+1}`;
  const mode=hasMode ? parts[1].trim() : '';
  const detail=hasMode ? parts.slice(2).join(' ').trim() : raw;
  const readable=detail
   .replace(/\s+(Time \d{2}:\d{2}\.)/g,'\n$1')
   .replace(/\s+(Official ranking update:)/g,'\n$1')
   .replace(/\s+(Official match:)/g,'\n$1')
   .replace(/\s+(NOTABLE EVENTS:)/g,'\n$1')
   .replace(/\s+(Ranking:)/g,'\n$1');
  const detailHtml=escapeHtml(readable).split('\n').filter(Boolean).map(line=>`<span>${line}</span>`).join('');
  return `<article class="savedResultCard"><div class="savedResultNumber">${idx+1}</div><div class="savedResultBody"><strong>${escapeHtml(title)}</strong>${mode?`<em>${escapeHtml(mode)}</em>`:''}${detailHtml}</div></article>`;
 }).join('')}</div>`;
}

function isShowComplete(){
 return eventCard.matches.length>0 && eventCard.matches.every(m=>!!m.result);
}

function renderShowCompletePanel(){
 const universeName=territories.find(x=>x.id===currentTerritory)?.name || currentTerritory;
 const ratings=eventCard.matches.map(m=>typeof m.rating==='number'?m.rating:0).filter(Boolean);
 const avg=ratings.length ? (ratings.reduce((a,b)=>a+b,0)/ratings.length).toFixed(2) : '—';
 const mainEvent=eventCard.matches[eventCard.matches.length-1];
 const mainEventLabel=mainEvent ? displayMatchLabel(mainEvent,eventCard.matches.length-1) : 'No main event';
 const motn=eventCard.matches
  .map((m,i)=>({m,i,r:typeof m.rating==='number'?m.rating:-1}))
  .sort((a,b)=>b.r-a.r)[0];
 const motnText=motn && motn.r>=0 ? `${displayMatchLabel(motn.m,motn.i)} · ${motn.r.toFixed(2)}★` : 'Not rated';
 return `<section class="showCompletePanel"><div class="showCompleteKicker">Show Complete</div><h2>${escapeHtml(eventCard.name || 'Untitled Card')}</h2><div class="showCompleteMeta">${escapeHtml(universeName)} · ${escapeHtml(eventCard.eventType)} · ${escapeHtml(eventCard.venue || 'Venue TBD')} · ${escapeHtml(eventCard.date || 'Date TBD')}</div><div class="showCompleteStats"><div><strong>${eventCard.matches.length}</strong><span>Matches Completed</span></div><div><strong>${avg}</strong><span>Average Rating</span></div></div><div class="showCompleteHighlights"><div><label>Main Event</label><strong>${escapeHtml(mainEventLabel)}</strong></div><div><label>Match of the Night</label><strong>${escapeHtml(motnText)}</strong></div></div><div class="cardBuilderActions showCompleteActions"><button id="startNextShow">Start Next Show</button><button id="returnFederation" class="secondary backToFederation">← Return to Federation</button></div><p class="tiny">Results remain below. Build 22 can archive this into Completed Shows history.</p></section>`;
}

function isUnbookedCardSlot(m:CardMatch){
 return !m.result && (!m.label || /^Match \d+$/.test(m.label));
}

function isActiveRosterWrestler(id?:string){
 return !!id && roster().some(w=>w.id===id);
}

function applySuggestion(a:string,b:string){
 const aId=normalizeWrestlerId(a) || a;
 const bId=normalizeWrestlerId(b) || b;
 if(!isActiveRosterWrestler(aId) || !isActiveRosterWrestler(bId) || aId===bId){
  alert('That suggested match is not valid for the selected roster/universe.');
  render();
  return;
 }
 const slot=eventCard.matches.find(isUnbookedCardSlot) || eventCard.matches.find(m=>!m.result) || eventCard.matches[0];
 if(!slot) return;
 slot.type='singles';
 slot.a=aId;
 slot.b=bId;
 slot.teamA=[aId];
 slot.teamB=[bId];
 slot.teamAPreset='manual';
 slot.teamBPreset='manual';
 slot.labelMode='auto';
 syncCardMatchParticipants(slot);
 updateAutoLabel(slot);
 openCardMatchId=slot.id;
 clearCardMatchResult(slot);
 saveUniverseState();
 render();
 requestAnimationFrame(()=>{
  const target=document.getElementById(`card-match-${slot.id}`);
  target?.scrollIntoView({behavior:'smooth',block:'start'});
  target?.classList.add('bookingMatchHighlight');
  window.setTimeout(()=>target?.classList.remove('bookingMatchHighlight'), 1200);
  const details=target?.querySelector<HTMLDetailsElement>('.bookingMatchDetails');
  if(details){
   document.querySelectorAll<HTMLDetailsElement>('.bookingMatchDetails').forEach(other=>{ if(other!==details) other.open=false; });
   details.open=true;
  }
 });
}
...
let openCardMatchId:number|null = null;

const UNIVERSE_SAVE_KEY = 'scPersistentUniverse_v1';
const EVENT_CARD_KEY = 'scEventCard_v1';
const EVENT_CARDS_KEY = 'scEventCardsByTerritory_v1';

type TerritoryCardMap = Partial<Record<TerritoryId, EventCard>>;
...
function normalizeEventCardForTerritory(card: EventCard | undefined, territory: TerritoryId): EventCard {
 const fallback = createDefaultEventCard(territory);
 const normalized = card ? { ...fallback, ...card } : fallback;
 normalized.territory = territory;
...
function readTerritoryCards(): TerritoryCardMap {
 try {
  return JSON.parse(localStorage.getItem(EVENT_CARDS_KEY) || '{}') as TerritoryCardMap;
 } catch {
  return {};
 }
}

function writeTerritoryCards(cards: TerritoryCardMap){
 try { localStorage.setItem(EVENT_CARDS_KEY, JSON.stringify(cards)); } catch {}
}

function saveEventCardForTerritory(territory: TerritoryId = currentTerritory, card: EventCard = eventCard){
 const cards = readTerritoryCards();
 cards[territory] = normalizeEventCardForTerritory(card, territory);
 writeTerritoryCards(cards);
}

function loadEventCardForTerritory(territory: TerritoryId): EventCard {
 const cards = readTerritoryCards();
 return normalizeEventCardForTerritory(cards[territory], territory);
}

function ensureActiveEventCard(){
 if(eventCard.territory !== currentTerritory){
  eventCard = loadEventCardForTerritory(currentTerritory);
 }
}
...
function saveEventCard(){
 try {
  ...
  saveEventCardForTerritory(currentTerritory, eventCard);
  localStorage.setItem(EVENT_CARD_KEY, JSON.stringify(eventCard));
 } catch {}
}
...
function hydrateSavedUniverse(){
 try {
  ...
  if(savedCard){
   const legacyCard = JSON.parse(savedCard) as EventCard;
   const legacyTerritory = legacyCard.territory || currentTerritory;
   const cards = readTerritoryCards();
   if(!cards[legacyTerritory]){
    cards[legacyTerritory] = normalizeEventCardForTerritory(legacyCard, legacyTerritory);
    writeTerritoryCards(cards);
   }
  }

  const raw = localStorage.getItem(UNIVERSE_SAVE_KEY);
  if(!raw){ eventCard = loadEventCardForTerritory(currentTerritory); return; }
  ...
  if(data.eventCards) writeTerritoryCards(data.eventCards);
  if(data.eventCard){
   const cardTerritory = data.eventCard.territory || currentTerritory;
   const cards = readTerritoryCards();
   if(!cards[cardTerritory]){
    cards[cardTerritory] = normalizeEventCardForTerritory(data.eventCard, cardTerritory);
    writeTerritoryCards(cards);
   }
  }
  eventCard = loadEventCardForTerritory(currentTerritory);
...
function importUniverseBackup(file: File){
 ...
 reader.onload = () => {
  try {
   ...
   if(data.eventCard){
    const cardTerritory = data.eventCard.territory || currentTerritory;
    saveEventCardForTerritory(cardTerritory, data.eventCard);
   }
   eventCard = loadEventCardForTerritory(currentTerritory);
   if(data.rosterRelations) rosterRelations = data.rosterRelations;
   if(data.feuds) feuds = data.feuds;
...
function resetUniverseSave(){
 ...
 if(!ok) return;
 [UNIVERSE_SAVE_KEY, EVENT_CARD_KEY, EVENT_CARDS_KEY, REL_KEY, HISTORY_KEY, FEUD_KEY, MATCHUP_KEY, STORAGE_KEY, 'scSelectedTerritory', 'scCurrentScreen', 'scCurrentView'].forEach(k=>localStorage.removeItem(k));
 location.reload();
}
...
function createCard(){
 ...
 currentMatch=null; openCardMatchId=1;
 for(let i=1;i<=n;i++) eventCard.matches.push({id:i,type:'singles',label:`Match ${i}`,labelMode:'auto',entrants:[], entrantMode:'random', entrantCount:20, mode:'quick', rule:'Standard'});
 saveUniverseState();
 render();
}

function resequenceCardMatches(){ eventCard.matches.forEach((m,i)=>{ m.id=i+1; if(!m.label || /^Match \d+$/.test(m.label)) { m.label=`Match ${i+1}`; m.labelMode='auto'; } }); }
...
function addCardMatch(){
 ...
 eventCard.matches.push({id:next,type:'singles',label:`Match ${next}`,labelMode:'auto',entrants:[], entrantMode:'random', entrantCount:20, mode:'quick', rule:'Standard'});
 openCardMatchId=next;
 saveUniverseState(); render();
}
function startNextShow(){
 const previous=eventCard;
 eventCard=createDefaultEventCard(currentTerritory);
 eventCard.eventType=previous.eventType || eventCard.eventType;
 eventCard.venue=previous.venue || eventCard.venue;
 eventCard.name='Next Card';
 eventCard.history=[];
 eventCard.matches=[];
 currentMatch=null;
 openCardMatchId=1;
...
function deleteCardMatch(id:number){
 ...
 if(openCardMatchId===id) openCardMatchId=null;
...
function moveCardMatch(id:number, dir:-1|1){
 ...
 const copy=[...eventCard.matches]; [copy[idx],copy[next]]=[copy[next],copy[idx]]; eventCard.matches=copy; openCardMatchId=id+dir; resequenceCardMatches(); saveUniverseState(); render();
}
function ensureCardTeams(m:CardMatch){
 ...
 if(size===1){ m.a=m.a || m.teamA[0]; m.b=m.b || m.teamB[0]; syncCardMatchParticipants(m); }
}
...
function updateCardMatch(id:number, field:string, val:string){
 ...
 if(field==='label'){ m.label=val; m.labelMode=val.trim() ? 'custom' : 'auto'; }
 ...
 syncCardMatchParticipants(m);
 if(field!=='label') updateAutoLabel(m);
 if(['type','a','b','rule','entrantMode','entrantCount','teamAPreset','teamBPreset'].includes(field) || /^teamA\d+$/.test(field) || /^teamB\d+$/.test(field)){ clearCardMatchResult(m); }
 saveUniverseState();
 render();
}
...
function cardParticipantOptions(selected?:string, currentId=1){
 const activeRoster=roster().sort((a,b)=>a.name.localeCompare(b.name));
 const selectedIsActive=!selected || isConditionalRef(selected) || activeRoster.some(w=>w.id===selected);
 const invalidSelected = selected && !selectedIsActive ? `<option value="${selected}" selected disabled>${wrestlerName(selected)} — not in selected roster</option>` : '';
 const base=invalidSelected + activeRoster.map(w=>`<option value="${w.id}" ${selected===w.id?'selected':''}>${w.name} — ${w.pp} PP / ${w.qpr}</option>`).join('');
...
function render(){
 ...
 const nav = `<nav class="heroNavBar" aria-label="Territory tools"><button class="tab ${currentView==='newMatch'?'active':''}" data-view="newMatch">New Match</button><button class="tab ${currentView==='cardBuilder'?'active':''}" data-view="cardBuilder">Book Show</button><button class="tab ${currentView==='rosterView'?'active':''}" data-view="rosterView">Roster View</button><button class="tab ${currentView==='rankings'?'active':''}" data-view="rankings">Rankings</button><button class="tab ${currentView==='feuds'?'active':''}" data-view="feuds">Feuds</button></nav>`;
 let body = '';
 ...
 document.getElementById('backHome')?.addEventListener('click',()=>{saveEventCardForTerritory(currentTerritory,eventCard); currentScreen='home'; localStorage.setItem('scCurrentScreen','home'); currentMatch=null; render();});
 ...
 document.getElementById('bookShowFromDashboard')?.addEventListener('click',()=>{ currentView='cardBuilder'; localStorage.setItem('scCurrentView',currentView); setupType='none'; render(); });
 ...
 document.getElementById('addCardMatchBottom')?.addEventListener('click',addCardMatch);
 ...
 document.getElementById('startNextShow')?.addEventListener('click',startNextShow);
 ...
 document.querySelectorAll('[data-autolabel]').forEach(el=>el.addEventListener('click',()=>{ const m=eventCard.matches.find(x=>x.id===Number((el as HTMLElement).dataset.autolabel)); if(m){ m.labelMode='auto'; updateAutoLabel(m); openCardMatchId=m.id; saveUniverseState(); render(); } }));
 ...
 document.querySelectorAll<HTMLDetailsElement>('.bookingMatchDetails').forEach(d=>d.addEventListener('toggle',()=>{ if(d.open){ const article=d.closest<HTMLElement>('.bookingMatchCard'); const id=article?.id.replace('card-match-',''); openCardMatchId=id?Number(id):openCardMatchId; document.querySelectorAll<HTMLDetailsElement>('.bookingMatchDetails').forEach(other=>{ if(other!==d) other.open=false; }); } }));
 document.querySelectorAll('[data-scrollmatch]').forEach(el=>el.addEventListener('click',()=>{ const target=document.getElementById(`card-match-${(el as HTMLElement).dataset.scrollmatch}`); target?.scrollIntoView({behavior:'smooth',block:'start'}); const details=target?.querySelector<HTMLDetailsElement>('.bookingMatchDetails'); if(details){ document.querySelectorAll<HTMLDetailsElement>('.bookingMatchDetails').forEach(other=>{ if(other!==details) other.open=false; }); details.open=true; openCardMatchId=Number((el as HTMLElement).dataset.scrollmatch); } }));
...
function renderBookingAdvisor(){
 ...
 return `<details class="advisorPanel bookingAdvisorCollapsed"><summary>Booking Advisor</summary><div class="scheduleHelper"><strong>Territory Scheduling Helper</strong><span>${scheduling}</span>${isHouseOrRegularTv()?'<em>House shows and regular TV tapings may include enhancement matches where a jobber/prelim wrestler is fed to a main-eventer or upper-card wrestler.</em>':''}</div>${warnBlock}<div class="suggestionList">${sugBlock || '<div class="tiny">Create a card or add feud/allies/enemies data for smarter suggestions.</div>'}</div></details>`;
}
...
function cardParticipantLabel(id?:string){
 if(!id) return '<span class="unbooked">TBD</span>';
 if(id.startsWith('winner:')) return `<span class="placeholderPick">Winner of Match ${id.split(':')[1]}</span>`;
 if(id.startsWith('loser:')) return `<span class="placeholderPick">Loser of Match ${id.split(':')[1]}</span>`;
 return wrestlerName(id);
}

function cardSideSummary(m:CardMatch, side:'A'|'B'){
 ensureCardTeams(m);
 if(isCardBattleType(m.type)) return '';
 const ids = side==='A' ? (m.teamA || [m.a || '']) : (m.teamB || [m.b || '']);
 const clean = ids.filter(Boolean);
 if(m.type==='singles') return cardParticipantLabel(clean[0]);
 return clean.length ? clean.map(cardParticipantLabel).join('<span class="teamJoiner"> & </span>') : '<span class="unbooked">TBD</span>';
}

function matchRoleLabel(m:CardMatch, index:number){
 return displayMatchLabel(m,index);
}

function renderCommonMatchEditor(m:CardMatch, typeOpts:string){
 return `<div class="bookingEditorGrid"><div><label>Match Label</label><div class="labelAutoRow"><input data-cardfield="label" data-id="${m.id}" value="${m.label}"><button type="button" class="secondary smallBtn" data-autolabel="${m.id}">Auto</button></div></div><div><label>Match Type</label><select data-cardfield="type" data-id="${m.id}">${typeOpts}</select></div><div><label>Play Mode</label><select data-cardfield="mode" data-id="${m.id}"><option value="quick" ${(m.mode||'quick')==='quick'?'selected':''}>Quick Match</option><option value="full" ${m.mode==='full'?'selected':''}>Full Match</option></select></div><div><label>Rules</label><select data-cardfield="rule" data-id="${m.id}">${ruleOptions(m.rule||'Standard')}</select></div></div>`;
}

function renderBattleEditor(m:CardMatch, typeOpts:string){
 const count=m.entrantCount||20;
 const mode=m.entrantMode||'random';
 const checklist = mode==='selected' ? `<div class="cardEntrantTools"><button class="secondary smallBtn" data-cardrandomentrants="${m.id}">Random Fill</button><button class="secondary smallBtn" data-cardclearentrants="${m.id}">Clear</button><span class="tiny">${(m.entrants||[]).length} selected</span></div><div class="cardEntrantChecklist bookingSheetEntrants">${roster().sort((a,b)=>a.rank-b.rank).map(w=>`<label class="cardEntrantRow"><input type="checkbox" data-cardentrant="${m.id}" value="${w.id}" ${(m.entrants||[]).includes(w.id)?'checked':''}><span>${w.name}</span><em>${w.pp} PP · BR ${w.brAdj>=0?'+':''}${w.brAdj}</em></label>`).join('')}</div>` : `<div class="tiny conditionalHelp">Random will pull ${count} different entrants from the current universe when the match is run.</div>`;
 return `${renderCommonMatchEditor(m,typeOpts)}<div class="bookingEditorGrid"><div><label>Entrants</label><select data-cardfield="entrantCount" data-id="${m.id}">${[6,8,10,12,15,20,25,30].map(n=>`<option value="${n}" ${count===n?'selected':''}>${n}</option>`).join('')}</select></div><div><label>Selection</label><select data-cardfield="entrantMode" data-id="${m.id}"><option value="random" ${mode==='random'?'selected':''}>Random</option><option value="selected" ${mode==='selected'?'selected':''}>Selected</option></select></div></div>${checklist}`;
}

function renderSinglesEditor(m:CardMatch, typeOpts:string){
 ensureCardTeams(m);
 return `${renderCommonMatchEditor(m,typeOpts)}<div class="bookingEditorGrid"><div><label>Wrestler A</label><select data-cardfield="a" data-id="${m.id}">${cardParticipantOptions(m.a||m.teamA?.[0],m.id)}</select></div><div><label>Wrestler B</label><select data-cardfield="b" data-id="${m.id}">${cardParticipantOptions(m.b||m.teamB?.[0],m.id)}</select></div></div>`;
}

function renderCardMatchSlot(m:CardMatch, typeOpts:string, index=0){
 syncCardMatchParticipants(m);
 ...
 const role=matchRoleLabel(m,index);
 const matchType=cardTypeName(m.type);
 const tools=`<div class="bookingCardTools"><button class="secondary smallBtn" data-cardmoveup="${m.id}">↑</button><button class="secondary smallBtn" data-cardmovedown="${m.id}">↓</button><button class="secondary smallBtn dangerLite" data-carddelete="${m.id}">Delete</button></div>`;
 const editBody = isBattle ? renderBattleEditor(m,typeOpts) : m.type==='singles' ? renderSinglesEditor(m,typeOpts) : `${renderCommonMatchEditor(m,typeOpts)}<div class="tiny conditionalHelp">${matchType} requires ${cardTeamSize(m.type)} wrestlers per side. Later matches can use Winner/Loser placeholders.</div>${renderTeamSelectors(m)}`;
 const summary = isBattle
  ? `<div class="bookingVersus"><strong>${m.entrantMode==='selected' ? `${(m.entrants||[]).length} selected entrants` : `${m.entrantCount||20} random entrants`}</strong></div>`
  : `<div class="bookingVersus"><strong>${cardSideSummary(m,'A')}</strong><span>vs.</span><strong>${cardSideSummary(m,'B')}</strong></div>`;
 const slotRole=slotPositionLabel(index);
 const openAttr = (openCardMatchId!==null ? m.id===openCardMatchId : index===0) ? ' open' : '';
 return `<article class="bookingMatchCard" id="card-match-${m.id}"><details class="bookingMatchDetails"${openAttr}><summary class="bookingCardHeader"><div class="bookingHeaderCenter"><div class="bookingRole">${role}</div><div class="bookingMeta">${slotRole} · ${matchType} · ${m.rule||'Standard'} · ${(m.mode||'quick')==='quick'?'Quick Match':'Full Match'}</div></div>${tools}</summary>${summary}<div class="bookingStatusRow">${result || '<span class="tiny">Not yet run.</span>'}</div><div class="bookingMatchEditor">${editBody}</div></details></article>`;
}

function renderCurrentCardSummary(){
 const rows = eventCard.matches.map((m,i)=>{
  syncCardMatchParticipants(m);
  const role=slotPositionLabel(i);
  const label=plainText(displayMatchLabel(m,i));
  const participants=isCardBattleType(m.type)
   ? (m.entrantMode==='selected' ? `${(m.entrants||[]).length} selected entrants` : `${m.entrantCount||20} random entrants`)
   : `${plainText(cardSideSummary(m,'A'))} vs ${plainText(cardSideSummary(m,'B'))}`;
  const detail = label.toLowerCase()===participants.toLowerCase() ? cardTypeName(m.type) : participants;
  const result=m.result ? ` — Winner: ${m.winnerId?wrestlerName(m.winnerId):m.result}` : '';
  return `<button class="cardSummaryRow" data-scrollmatch="${m.id}"><strong>${i+1}. ${role}</strong><span><b>${escapeHtml(label)}</b><em>${escapeHtml(detail + result)}</em></span></button>`;
 }).join('');
 return `<section class="currentCardSummary"><h3>Current Card</h3>${rows || '<div class="tiny">No matches booked yet.</div>'}</section>`;
}
...
<option value="survivorSeries5" ${m.type==='survivorSeries5'?'selected':''}>Survivor Series 5 vs 5</option><option value="battleRoyal" ${m.type==='battleRoyal'?'selected':''}>Battle Royal</option><option value="royalRumble" ${m.type==='royalRumble'?'selected':''}>Royal Rumble</option><option value="tagBattleRoyal" ${m.type==='tagBattleRoyal'?'selected':''}>Tag Team Battle Royal</option><option value="bunkhouseBattleRoyal" ${m.type==='bunkhouseBattleRoyal'?'selected':''}>Bunkhouse Battle Royal</option><option value="warGames" ${m.type==='warGames'?'selected':''}>WarGames</option>`;
 const universeName=territories.find(x=>x.id===currentTerritory)?.name || currentTerritory;
 const hero=`<div class="bookingPageTop"><button id="returnFederation" class="secondary backToFederation">← Return to Federation</button></div><div class="bookingHero"><div><span class="eyebrow">${universeName}</span><h2>${eventCard.name || 'Untitled Card'}</h2><div class="bookingHeroMeta">${eventCard.eventType} · ${eventCard.venue || 'Venue TBD'} · ${eventCard.date || 'Date TBD'}</div></div><div class="bookingHeroCount"><strong>${eventCard.matches.length}</strong><span>matches</span></div></div>`;
 if(isShowComplete()){
  return `<div class="card bookingSheetPage showCompleteMode">${hero}${renderShowCompletePanel()}<h3>Saved Results</h3>${renderSavedResults()}</div>`;
 }
 const firstRunnable=eventCard.matches.find(m=>!m.result);
 const runNext=firstRunnable ? `<button class="secondary" data-cardquick="${firstRunnable.id}">Run Next Match</button>` : '<button class="secondary" disabled>Show Complete</button>';
 return `<div class="card bookingSheetPage">${hero}<details class="showSetupPanel"><summary>Show Setup</summary><div class="formrow"><div><label>Show Name</label><input id="cardName" value="${eventCard.name}"></div><div><label>Event Type</label><select id="cardEventType">${eventOpts}</select></div></div><div class="formrow"><div><label>Date</label><input id="cardDate" value="${eventCard.date}"></div><div><label>Venue</label><select id="cardVenue">${venueOptions()}</select></div></div><div class="formrow"><div><label>Matches</label><select id="cardMatches">${[5,6,7,8,9,10,12].map(n=>`<option ${eventCard.matches.length===n?'selected':''}>${n}</option>`).join('')}</select></div><div><label>Current Universe</label><div class="staticField">${universeName}</div></div></div><div class="tiny conditionalHelp">Later matches can use card-result placeholders like Winner of Match 1 / Loser of Match 1. Save Card keeps the card in this universe so you can return and keep editing.</div></details><div class="cardBuilderActions bookingToolbar"><button id="createCard">Create / Reset Card</button><button id="addCardMatch" class="secondary">Add Match</button>${runNext}<button id="saveCard" class="secondary">Save Card</button></div><section class="bookingSheetList">${eventCard.matches.map((m,i)=>renderCardMatchSlot(m,typeOpts(m),i)).join('') || '<div class="emptyBookingSheet">No matches booked yet. Add a match or create a card.</div>'}</section>${renderCurrentCardSummary()}${renderBookingAdvisor()}<div class="cardBuilderActions bottom bookingToolbar"><button id="addCardMatchBottom" class="secondary">Add Match</button>${runNext}<button id="saveCardBottom" class="secondary">Save Card</button></div><h3>Saved Results</h3>${renderSavedResults()}</div>`;
}
